...
//	echo "</pre>";	//if(isset($_SESSION['mdTFields']))
					...
					echo genORen();
					echo genOCol('','','','',$suma_width+$widthEditable);
					echo genOTable('0',$suma_width+$widthEditable,'','',$CFG_BGC[1],'1','0');
					...
					echo genCol($strvalcol,'2','','',$CFG_BGC[2],'','',$CFG_LBL[25]);
					echo genCRen();
					echo genCTable().genCCol().genCRen();
					//
...
if (trim($a_key[0])=='100' && !$boolNoEditable && !$boolNoNew){
	...
	echo genORen();
	echo genCol(genOTable('0','44','','',$CFG_BGC[10],'0','1','left').genORen()
	...
	echo genORen();
	echo genOCol('','','','',$suma_width+$widthEditable);
	echo genOTable('0',$suma_width+$widthEditable,'','',$CFG_BGC[3],'2','1');
	/*titulos de entidad*/
			$conta=0;
			foreach ($field as $afield1){
			$conta++;
			}
			if (!in_array($afield1[0],$array_noprint_ent) && $afield1[2]=='1')
				echo genCol(isset($afield1[17])?$afield1[17]:'','2','','',(isset($afield1[20]) && strlen($afield1[20])>0)?$afield1[20]:'',
				'','',$CFG_LBL[28],'',(isset($afield1[29]) && strlen($afield1[29])>0)?$afield1[29]:'background="img/bg02.jpg"');
		...
		foreach ($field as $afield)
			if (!in_array($afield[0],$array_noprint_ent) && $afield[2]=='1')
				echo genCol(isset($afield[17])?$afield[17]:$afield[1],'','','',(isset($afield[20]) && strlen($afield[20])>0)?$afield[20]:'',
								$afield[8],'',$CFG_LBL[22],'',(isset($afield[29]) && strlen($afield[29])>0)?$afield[29]:'background="img/bg02.jpg"');
		echo genCRen();
