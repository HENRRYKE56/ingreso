...
function AddPage($orientation='', $format='')
{
	...
	if($this->page>0)
	{
		//Page footer
		$this->InFooter=true;
        if($hoja_blanco!=1)      $this->Footer();         
		else {
                                        $this->SetDrawColor(0,0,0);
					$this->SetFont('Arial','',8);
					$this->SetTextColor( 0 , 0 , 0 ) ;
                                        $this->SetXY(10,160);
                                        $this->MultiCell (71,5,'Secretaría de Salud
Instituto Mexiquense contra las Adicciones
El presente documento queda registrado en: 

Libro_______ hoja_______ folio__________
Fecha. Día___ de __________de________

Área. Dirección del Instituto Mexiquense contra las Adicciones   
',1,'C',false);   
                }
		$this->InFooter=false;
		//Close page
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			$tam_real=$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
			$i++;
			$sep=-1;
			$j=$i;
			$l=0;
		...
		if($l>$wmax)
		{
			...
			else
			{
				...
				$tam_real=$this->Cell($w,$h,substr($s,$j,$sep-$j),$b,2,$align,$fill);
				$i=$sep+1;
			}
			$sep=-1;
			$j=$i;
