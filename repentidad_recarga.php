...
//}
    ...
    echo' <div class="breadcrumb-holder container-fluid">
            <ul class="breadcrumb" style="margin-bottom: 0px;padding: 0 0 0 0;">
              <li class="breadcrumb-item active h6 font-weight-bold">'.$entidad.'</li>
            </ul>
          </div>
          ';     
        $str_requeridos = "Los campos con&nbsp;&nbsp;<i class='fa fa-asterisk' aria-hidden='true' style='color:red;'></i>&nbsp;&nbsp;son obligatorios";
        $requeridos = '<div class="alert alert-success" role="alert" style="padding: 0 0 0 0;margin-top: 0px;">
                                ' . $str_requeridos . '
                            </div>';
        echo $requeridos;    
        $frmname = 'frmcon';
        ...
        for ($index2 = 0; $index2 < count($a_separs); $index2++) {
            $pinta_form.= '<div class="card">
                                            <div style="padding:2px 2px 2px 2px;" class="card-header text-light ' . $a_separs[$index2][3] . '" onclick="oculta(\'a_'.$a_separs[$index2][0].'\',\'i_'.$a_separs[$index2][0].'\')">
                                                <strong><i class="fa fa-minus-square text-danger" id="i_'.$a_separs[$index2][0].'" aria-hidden="true"></i>&nbsp;' . $a_separs[$index2][1] . '</strong>
                                            </div>  ';
            $pinta_form.= '<div style="padding:.2em .2em .2em 1em;" class="card-body card-block" id="a_'.$a_separs[$index2][0].'">';      
...
//                      </div>';                
        ...
        echo '          
                            <div class="col-12 col-sm-12 bg-white">'."
                                <button id=\"inicio\" class=\"btn btn-success\" alt=\"Generar Reporte\" border='0' value=\"\"" . " $vars_obj><i class='fa fa-play'></i>&nbsp;Inicio</button>".'
'.                              ("<button id=\"parar\" onclick='parar_recarga();' class=\"btn btn-danger\" disabled  ><i class='fa fa-stop'></i>&nbsp;Parar</button>&nbsp;<i id='reloj' style='border-radius: 10%;' class='text-dark font-weight-bold bg-light' disabled>0seg</i>"
        ).'                 </div>
                        ';
        ...
        if (1==2) {
            ...
            $iframe = "<div id=\"miiframe\" name=\"miiframe\" style=\"border: 1px solid #E0E4D1;z-index:10; overflow:scroll; height:500px;\"></div>" .
