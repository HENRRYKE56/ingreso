...
if ($classval->NumReg>0){
	...
	$entidad='PERFIL USERGROUP';
	$strSelf="sb_perfil_usergroup.php";
	$id_prin='cve_usergroup';
	$strWhere="Where ";
	$a_order=array();
	switch ($__SESSION->getValueSession('cveperfil')){
		case 1:
			$tablas_c='sb_perfil_usergroup';
			$a_order[]=$id_prin;
			break;
	...
	$boolkeyValNiv=TRUE;
	$a_mdLabel=array('grupo de usuarios');
	$m_keyFields=array('cve_usergroup');
	$m_Table='sb_usergroup';
	$m_Fields=array('cve_usergroup','des_usergroup');
	$m_keyTypeFields=array('num');	//num,text
	$m_TypeFields=array('num','text');	//num,text
	$a_keyValNiv=array();
	...
	//$items0='perfil.*';
	$tabla='sb_perfil_usergroup';
        $strDistintic="SELECT count(*) as count_r FROM ".$tabla." ".$strWhere;
	$intlimit=8;
	$a_separs=array();
	$a_separs[]=array(1,'datos generales',6,'separ_verde');
	$field[]=array('cve_usergroup','usergroup','2','label','0','','int',0,50,20,0,'',1,1,5);
	$field[]=array('cve_perfil','Perfil','1','select','2',array($per_val,$per_des),'int',0,700,20,2,'',1,array(1,'col-12 col-sm-12 col-12 col-sm-12'),array(1,1,6));
	$setdel='';
	$strdistintic="sb_perfil_usergroup.cve_usergroup, sb_perfil_usergroup.cve_perfil";
	
	$keyFields=array('cve_usergroup','cve_perfil');
	$keyFieldsUnique=array('cve_usergroup','cve_perfil');
