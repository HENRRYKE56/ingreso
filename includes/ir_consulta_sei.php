...
if ($str_check) {
    ...
    if ($classval->NumReg > 0) {
        /* ----------JURISDICCION--------------------- */
        $obligatorio = 0;
        $entidad = "CONSULTA DE INFORMACION DE POSIBLES POSITIVOS";
...
$fecha_ini=date("d/m/Y");        
        $a_separs = array();
        $a_separs[] = array(0, 'Filtros de informaci�n', 6, 'separ_verde');
        $field[] = array('idopc', 'idopc', '1', 'hidden', '1', '', 'int', 1, 200, 10, 2, '', 0, 1, 1, '');

        $field[] = array('fecha_visita', 'Fecha de visita', '1', 'text', '1', '', 'date', $fecha_ini, 100, 10, 0, '', 1, array(1, 'col-12 col-md-12', 'col-12 col-md-6'), array(1, 1, 1), '', 0, 'Fecha', 0, '',
            '', '', '', '', '', /* conf date jquery */ $conf_date, /* funcion */ $conf_funciones);
//        $field[] = array('fecha_fin', 'A:', '1', 'text', '1', '', 'date', $fecha_ini, 100, 10, 0, '', 1, array(1, 'col-12 col-md-12', 'col-12 col-md-4'), array(1, 1, 1), '', 0, 'Fecha', 0, '',
//            '', '', '', '', '', /* conf date jquery */ $conf_date, /* funcion */ $conf_funciones);
        $field[] = array('telefono', 'Telefono', '1', 'text', '1', '', 'char', '', 100, 10, 20, '', 0, array(1, 'col-12 col-sm-12', 'col-12 col-sm-6'), array(1, 1, 1), '', 0, '', 0, '');

        $a_check = array(); // si un elemento empieza por uno  los elementos siempre se cosideraran para el reporte sean obligatorios o no 
        $a_check[] = array(1, array('idopc', 'fecha_visita','telefono'));
