...
if ($classval->NumReg>0){
	...
	$classconsul->ListaEntidades(array('cve_perfil'),'perfil','');
	...
	$strSelf="i_perfil_usergroup.php";
	$id_prin='cve_usergroup';
	$strWhere="Where";
	$a_order=array();
	switch ($_SESSION['cveperfil']){
		case 1:
			$tablas_c='perfil_usergroup';
			$a_order[]=$id_prin;
			break;
		case $_SESSION['cveperfil']<>1:
			$strWhere.=" cve_usergroup=".$_SESSION['cveusergroup'];
			$tablas_c='perfil_usergroup';
			$a_order[]=$id_prin;
			break;
	}
	$boolkeyValNiv=TRUE;
	$a_mdLabel=array('usergroup');
	...
	$setdel='';
	$strdistintic="perfil_usergroup.cve_usergroup, perfil_usergroup.cve_perfil";
	
	$keyFields=array('cve_usergroup','cve_perfil');
	$keyFieldsUnique=array('cve_usergroup');
