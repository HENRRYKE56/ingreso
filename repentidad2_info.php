...
//                            timeFormat: 'HH:mm',";                    
        ...
        echo '<nav aria-label="breadcrumb" class="bread_sys">
            <ol class="breadcrumb" style="background-color:#4c4c4c;">
                <li class="breadcrumb-item active" style="color:#ffffff;" aria-current="page"><h2 class="h6"><i class="fa fa-home" style="font-size:1.3em;color:#7fffd4;"></i>&nbsp;' . $entidad . '</h2></li>
            </ol>
          </nav>';
        ...
        for ($index2 = 0; $index2 < count($a_separs); $index2++) {
            ...
            $pinta_form.= '<div class="card div_form_sys">
                <h3 class="color_negro h3">
                                            <button aria-label="' . ($ariaexpanded == "true" ? 'Contraer' : 'Expandir') . '" aria-expanded="' . $ariaexpanded . '" class="btn card-header ' . $a_separs[$index2][3] . ' Accordion-trigger punteados w-100 b_s_sys" aria-controls="a_' . $a_separs[$index2][0] . '" id="accordion' . ($a_separs[$index2][0]) . 'id">
                                                <span style="width:50% !important;pointer-events: none;text-align:left !important;color:inherit;background-color:inherit;" class="font-weight-bold Accordion-title">&nbsp;' . $a_separs[$index2][1] . '</span><span style="width:50% !important;text-align: right !important;pointer-events: none;" class="Accordion-icon"><i class="' . ($ariaexpanded == "true" ? 'fa fa-chevron-up' : 'fa fa-chevron-down') . '" id="i_' . $a_separs[$index2][0] . '" aria-hidden="true"></i></span>
                                            </button>  
                </h3>';
            ...
            if ($afield[4] == '1' || $afield[4] == '2') {
                if ($afield[3] == 'hidden') {
                    ...
                    if($afield[0]=='cve_persona')
                    $pinta_form.=$informacion_reporte;
                } else {
                        $l_des = "";
                        $span_des = "";
                        $descripcion = "";
                        ...
                        $pinta_form.= '<div class="row ' . $afield[13][2] . '" id="div_' . $afield[0] . '">
                                        <div class="col-12" style="height: 30px;">'.("").'<label for="' . $afield[0] . '" id="l_' . $afield[0] . '" class="form-control-label f_cmb_l label_sys" ' . $l_des . '>' .
                        ...
                        switch ($afield[3]) {
                            ...
                            case 'file':
                                $str_obj_jsrpt.=(strlen($str_obj_jsrpt) > 0 ? ', ' : '');
                                $str_obj_jsrpt.="{'name':'" . $afield[0] . "','type':'" . $afield[3] . "'}";
                                $pinta_form.=genInput('file', $afield[0], $afield[7], '', $afield[9], '', 'form-control-file input_text_sys', '', '', '9', '', '', '', '0', '', (isset($afield[24]) ? $afield[24] : '') . ' onKeyPress="return objKeyPressed(event,this,\'' . $frmname . '\');"') .
                                        '<small class="form-text" style="" id="e_' . $afield[0] . '">&nbsp;</small>';
                                break;
                            case 'select':
                                $str_obj_jsrpt.=(strlen($str_obj_jsrpt) > 0 ? ', ' : '');
                                ...
                                if (isset($afield[5][4]) && sizeof($afield[5][4]) > 0) {
                                    ...
                                    foreach ($afield[5][4] as $item_texts_a) {
                                        ...
                                        $pinta_form.= genInput('txtbox', $item_texts_a[1], '', '', $item_texts_a[3], $item_texts_a[4], 'txtbox input_text_sys', '', '', ($afield[6] == 'date' ? '1' : ($afield[6] == 'int' || $afield[6] == 'money' ? '2' : '0')), $afield[1], ($afield[6] == 'int' || $afield[6] == 'money' ? $afield[7] : ''), '', $cont, (isset($afield[15]) ? $afield[15] : ''), $item_texts_a[2]) . '<br>';
                                        $cont_txts++;
                                    }
                                }
                                //$("#cveArea").select2();
                            ...
                            case 'check':
                                ...
                                if (isset($afield[16]) && $afield[16] == 1) {
                                    ...
                                    $pinta_form.= genInput('checkbox', $afield[0], ((strlen($afield[5]) > 0) ? $afield[7] : '1'), '', '', '', ' checks_forms input_text_sys checks_forms_sys ', '', (''), '8', '', '', '', '0', '', (isset($afield[24]) ? $afield[24] : '') . ' onKeyPress="return objKeyPressed(event,this,\'' . $frmname . '\');"' . $accion) . '<span style="font-size: 11px;">&nbsp;' . $afield[1] . '&nbsp;</span>';
                                } else {
                                    $pinta_form.= genInput('checkbox', $afield[0], ((strlen($afield[5]) > 0) ? $afield[7] : '1'), '', '', '', ' checks_forms input_text_sys checks_forms_sys ', '', (''), '8', '', '', '', '0', '', (isset($afield[24]) ? $afield[24] : '') . ' onKeyPress="return objKeyPressed(event,this,\'' . $frmname . '\');"' . $accion);
                                }
...
'.        "<button id=\"submit\" class=\"btn boton_act punteados b_s_sys\" type=\"button\" alt=\"Generar Folio\" border='0' name=\"submit\" value=\"\"" . " $vars_obj><i class='fa fa-download'></i>&nbsp;Actualizar</button>" . '
                        ' . ("").'
                          </div>
                        </div></div>';                
...
//                      </div>';                
        ...
        if (isset($ValsendRep2) && $ValsendRep2 <> "sendRep2") {
            ...
            $iframe = "<div id=\"miiframe\" name=\"miiframe\" style=\"border: 1px solid #E0E4D1;z-index:10; overflow:scroll; height:400px;\">".$tabla_pintar."</div>" .
