    ...
    function Adicionar($avalues, $afields, $atypes, $tabla, $operation = "INSERT") {
        ...
        foreach ($avalues as $value) {
            ...
            if ($atypes[$cont] == 'money' || $atypes[$cont] == 'int') {
                ...
                if ($atypes[$cont] == 'char') {
                    $this->Con->Sql .= "'" . ($afields[$cont] == "passwd" ? md5(addslashes($value)) : addslashes($value)) . "'";
                } else {
                    $this->Con->Sql .= "'" . addslashes($value) . "'";
                }
            }
            $cont+=1;
        }
        ...
        foreach ($afields as $item) {
            ...
            if (strlen(trim($avalues[$cont])) > 0) {
                if ($atypes[$cont] == 'money' || $atypes[$cont] == 'int') {
                    ...
                    if ($atypes[$cont] == 'char') {
                        $this->Con->Sql .= "'" . ($afields[$cont] == "passwd" ? md5(addslashes($avalues[$cont])) : addslashes($avalues[$cont])) . "'";
                    } else {
                        $this->Con->Sql .= "'" . addslashes($avalues[$cont]) . "'";
                    }
                }
            } else {
                $this->Con->Sql .= 'NULL';
