...
//        print_r($classent);die();
    ...
    if ($intpag_s[$pag_id] > 1) {
        $primeros = "<li class='page-item punteados b_s_sys'><a href='#' aria-label='Primera p&aacute;gina' class='page-link paginacion_m rounded' onClick=\"window.location='" . $_SERVER['PHP_SELF'] . "?intpag_s" . $pag_id . "=1" . "'\" ><i class='fa fa-fast-backward' style='color:inherit;'></i></a></li>";
        $primeros.="<li class='page-item punteados b_s_sys'><a href='#' aria-label='Retroceder p&aacute;gina' class='page-link paginacion_m rounded' onClick=\"window.location='" . $_SERVER['PHP_SELF'] . "?intpag_s" . $pag_id . "=" . ($intpag[$pag_id] - 1) . "'\" ><i class='fa fa-backward' style='color:inherit;'></i></a></li>";
    } else {
    ...
    if ($intpag_s[$pag_id] < $decdiv) {
        $segundos = "<li class='page-item punteados b_s_sys'><a href='#' aria-label='Siguiente p&aacute;gina' class='page-link paginacion_m rounded' onClick=\"window.location='" . $_SERVER['PHP_SELF'] . "?intpag_s" . $pag_id . "=" . ($intpag[$pag_id] + 1) . "'\"><i class='fa fa-forward'></i></a></li>";
        $segundos.="<li class='page-item punteados b_s_sys'><a href='#' aria-label='Ultima p&aacute;gina' class='page-link paginacion_m rounded' onClick=\"window.location='" . $_SERVER['PHP_SELF'] . "?intpag_s" . $pag_id . "=" . $decdiv . "'\" ><i class='fa fa-fast-forward'></i></a></li>";
    } else {
   ...
   $barra_busqueda_only='';
    if (isset($a_search)) {
    $barra_busqueda='<div class="card search_sys">
                        <div class="row align-items-center">
                            <div class="col-auto">';
        if (trim($a_key[0]) == '100' && !$boolNoEditable && !$boolNoNew) {
            $barra_busqueda.= "<div style='width:100%;height:100%;text-align:center;'>";
            ...
            $barra_busqueda.=genInput('hidden', array('nivPo_new','nivPo_00'), (strlen($__SESSION->getValueSession('niv')) > 0) ? $__SESSION->getValueSession('niv') : '0');
            $barra_busqueda.=genInput('hidden', 'btnAdd', 'btnAdd');
            $barra_busqueda.= '<button style="height:100% !important;background-color: #008000;border: none;padding: 1px 1px;color:#8b4513;" type="submit" class="btn punteados b_s_sys"><i class="fa fa-plus-circle text-white" aria-hidden="true"></i>&nbsp;<span class="text-white" >Nuevo registro</span></button>';
            $barra_busqueda.="</fieldset></form></div>";
        }          
            $barra_busqueda.='  </div>';         
        if ($__SESSION->getValueSession('valSearch') <> "") {
            $strObjSearch = genInput('text', 'txtsearch', ($__SESSION->getValueSession('valSearch')), '', '40', '80', 'form-control input_text_sys', '', '', '0', 'Valor', '', '', '');
            if ($__SESSION->getValueSession('valSearch') <> "" && isset($a_search[4][$__SESSION->getValueSession('itemSearch')]) && $a_search[4][$__SESSION->getValueSession('itemSearch')] == 'select') {
                $strObjSearch = genSelect('txtsearch', '', '', $array_search_val, $array_search_des, ($__SESSION->getValueSession('valSearch')), '', '', ' class="form-control input_text_sys" ', '', '');
            }
        } else {
            $strObjSearch = genInput('text', 'txtsearch', '', '', '40', '80', 'form-control input_text_sys', '', '', '0', 'Valor', '', '', '');
            if (isset($a_search[0]) && isset($a_search[4][$a_search[0][0]]) && $a_search[4][$a_search[0][0]] == 'select') {
                $strObjSearch = genSelect('txtsearch', '', '', $array_search_val, $array_search_des, $a_search[0][0], '', '', ' class="form-control input_text_sys" ', '', '');
            }
        }
        ...
        if (isset($srchperiodo) && $srchperiodo == 1) {
            ...
            $strsrchperiodo = '<div class="col-12 col-md-1">' .
                    ...
                    '<div class="input-group col-12 col-md-2">' . genInput('text', isset($conf_date_b[0][0]) ? $conf_date_b[0][0] : 'txtinicio', (is_array($atmp_asrchperiodo) ? $atmp_asrchperiodo['inicio'] : date("d/m/Y")), '', '10', '10', 'form-control h5 input_text_sys', 'readonly', '', '6', $frmname, '', '', 1) . ''
                    . '   <div class="input-group-append">
                            <button type="button" class="btn btn-success b_s_sys" onclick="mensaje(\'' . (isset($conf_date_b[0][0]) ? $conf_date_b[0][0] : 'txtinicio') . '\')"><i class="fa fa-calendar text-white" style="font-size: 1rem;"></i></button>
                        </div>  
                    </div>' .
                    '<div class="col col-md-1"><label for="text-input" class=" form-control-label">F. Fin</label>' . '</div>' .
                    '<div class="input-group col-12 col-md-2">' . genInput('text', isset($conf_date_b[1][0]) ? $conf_date_b[1][0] : 'txtfin', (is_array($atmp_asrchperiodo) ? $atmp_asrchperiodo['fin'] : date("d/m/Y")), '', '10', '10', 'form-control h5 input_text_sys', 'readonly', '', '6', $frmname, '', '', 2) .
                    '<div class="input-group-append">
                            <button type="button" class="btn btn-success b_s_sys" onclick="mensaje(\'' . (isset($conf_date_b[1][0]) ? $conf_date_b[1][0] : 'txtfin') . '\')"><i class="fa fa-calendar text-white" style="font-size: 1rem;"></i></button>
                        </div>'
                    . '</div>' .
                    "";
        }
        $barra_busqueda.='  <div class="col-12 col-md-8">
                                    ...
                                    <fieldset><legend id = "legend_f_b_entidad" style="display:none;" >Busqueda en ' . $entidad . '</legend>
                                        <div class="row form-group" style="margin-bottom: 0px;">
                                            <div class="row">
                                                ...
                                                <label tabindex="0" style="padding-right: 6px;padding-left: 20px;">Buscar:</label>
                                                </div>
                                ...
                                ' onChange="reloadObjSearch(\'search\',\'content_search\',\'selsearch\',2,0,\'txtsearch\');"' : '') . (' class="col input_text_sys" style="margin-right:20px" ')) . '
                                                <div class="input-group col-12 col-sm-6">
                                                    ' . $strObjSearch . '
                                                    <div class="input-group-append">
                                                        <button type="submit" class="btn btn-warning b_s_sys" data-toggle="tooltip" title="Filtrar"><i class="fa fa-filter text-white" style="font-size: 1rem;"></i></button>
                                                    </div>                                                        
                                                </div>      
                                                    ' . $strsrchperiodo . '
    ...
    .'</div>';        
        ...
        if (trim($a_key[0]) == '100' && !$boolNoEditable && !$boolNoNew) {
            ...
            $barra_busqueda_only.=genInput('hidden', array('nivPo_new','nivPo_02'), (strlen($__SESSION->getValueSession('niv')) > 0) ? $__SESSION->getValueSession('niv') : '0');
...
//    print_r($niveles_a);die();
    ...
    echo '<div class="head-table">Listado de: '.$entidad.'</div><div class="card div_tabla_sys">
    ...
    if ($boolColHeaders) {
        ...
        echo genOTable('0', '', '', 'table table-hover  color_negro tabla_sys_en', '#FFFFFF', '0', '3', '', ((($i + 1) == $classent->NumReg) ? '' : 'margin-bottom:' . (isset($str_margin_reg) ? $str_margin_reg : "10px") . ';'));
        echo $barra_busqueda_only."<caption style='display:none;'>Listado de: ".$entidad."</caption>";
...
//		echo "</pre>";   
        ...
        echo genORen('', '', '', 'table_hover_cab table_hover_sys', ($i % 2) ? $strColorRow1 : $strColorRow2,'', "row_" . $i);
            ...
            if ((isset($intNivel) && $intNivel > 0) || sizeof($a_print) > 0) {
                ...
                for ($nx = 1; $nx <= 100; $nx++) {
                    ...
                    if (isset($$stridniv)) {
                        ...
                        $icon_niveles.="<a style='display: inline-block;' class='dropdown-item punteados a_sub_nivel_sys' id='menu_sub_n_".$nx."' href='" . $str_href . $trs_ref_add . "'><span><i id='sub_icon_".$i."_".$nx."' style='" . $var_n_e . "' class='" . (isset($$stridniv) ? $$stridniv : "") .  "'></i>&nbsp;&nbsp;" . ($var_n) . "</span></a>";
                    }
                }
...
//---Empiezan botones de edicion
        if ($b_printline == 1 && !$boolNoUpdate && $widthEditable > 0) {
            ...
            echo genInput('hidden',array('nivPo_u','nivPo_u_'.$i), (strlen($__SESSION->getValueSession('niv')) > 0) ? $__SESSION->getValueSession('niv') : '0');
            echo genInput('hidden',array('btnUpd','btnUpd_u_'.$i), 'btnUpd', '', '', '', 'btn') . "<button type='submit' class='bg-light boder b_s_sys' aria-label='Editar registro' title='Editar registro' data-toggle='tooltip' data-placement='right' ><i id='botones_accion_abc_".$i."' class='fa fa-pencil-square-o'></i></button>";
            //echo genCCol();
            echo $str_closeform;
        }
...
//						 $CFG_BGC[12]. "'; "."\" onMouseOut=\"this.style.backgroundColor='';\"");
            ...
            echo genInput('hidden',array('nivPo_d','nivPo_d_'.$i), (strlen($__SESSION->getValueSession('niv')) > 0) ? $__SESSION->getValueSession('niv') : '0');
            echo genInput('hidden',array('btnDel','btnDel_d_'.$i), 'btnDel', '', '', '', 'btn') .
            "<button type=\"submit\" value=\"\" title='Borrar registro' aria-label='Borrar registro' data-toggle='tooltip' data-placement='right' class='b_s_sys' onclick=\"show_confirm('MODULO DE " . strtoupper($entidad) . " : < mensaje de confirmacion >','Esta seguro que desea borrar el registro" . addslashes($strIdentifyRow) . "?');return document.obj_retVal;\"><i id='botones_accion_abc' class='fa fa-eraser'></i></button>";
//				echo genCCol();
            echo $str_closeform;
        }
        echo "</div>";
...
//--   termina columna de botones de edicion
    ...
    . '</section></div>';
    //paginacion
    $barra_registros = '       <div class="card card_sys">
                                <div class="card-body card-block" id="d_gen" style="padding-top: .5rem;padding-bottom: 0rem;">
							     ...
							     ' . genInput('text', 'intpag_s' . $pag_id, '', '', '5', '5', 'form-control input_text_sys', '', '', '0', 'Valor', '', '', '', '', 'style="width:20px;" aria-label="N&uacute;mero de p&aacute;gina"') .
            genInput('hidden', 'nivPo_p', (strlen($__SESSION->getValueSession('niv')) > 0) ? $__SESSION->getValueSession('niv') : '0') . '
                                                            <div class="input-group-append">
                                                                <button type="submit" class="btn paginacion_m b_s_sys" title="Buscar n&uacute;mero de p&aacute;gina">Ir</button>
                                                            </div>  
                                                        </div>
                                                    </div>                                                    
                                                </div>                                                    
                                                ...
                                                <div class="col-12 col-md-3 text-right">
                                                    <p class="color_negro label_sys">' . $reg_found . " registro(s) encontrado(s)" . '</p>
                                                </div>';
