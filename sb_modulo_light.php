...
//	}
	...
	$tabla='periodo';
	$a_separs=array();
	$a_separs[]=array(3,'PERIODO',2,"bg-success");
	$strDistintic="SELECT count(id_periodo) as count_r FROM periodo";
	$intlimit=20;
	$field[]=array('id_periodo','Clave','1','text','1','','int',0,100,100,0,'',1,array(1,'col-12 col-sm-12','col-12 col-sm-12'));
	$field[]=array('id_estatus_periodo','Estatus periodo','1','select','1',array($tipo_val,$tipo_des),'int',2,120,20,2,'',1,array(1,'col-12 col-sm-12','col-12 col-sm-12'),2,'','clase');	
	
	$setdel='';
	$keyFields=array('id_periodo');
	$keyTypeFields=array('num');	//int,text
	$array_noprint_ent=array('null');
	$keyFieldsUnique=array('id_periodo');
...
function fnImplementInUpd(){
	//if (isset($_POST['cveCorrespondencia']) && isset($_POST['cveAsignacion']) && isset($_POST['cveEstatus'])){
//			$array_namesxTmp=array('cveEstatus');
//			$array_typetxTmp=array('int');
//			$array_valuesxTmp=array($_POST['cveEstatus']);
//			$array_defaultxTmp=array(0);
//			$classent = new Entidad($array_namesxTmp,$array_defaultxTmp);
//			$classent->Modificar($array_valuesxTmp,$array_namesxTmp,$array_typetxTmp,'correspondencia'," cveCorrespondencia=".$_POST['cveCorrespondencia']);
			echo 'Inactivo'; 
//	}
 }
?>